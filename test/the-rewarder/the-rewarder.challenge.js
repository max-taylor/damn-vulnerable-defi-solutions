...
describe("[Challenge] The rewarder", function () {
  ...
  // Exploit works by waiting until the next reward round, taking out a flash loan of 1mil DVT, depositing the tokens into the rewarder pool, distributing rewards (thereby taking a new token snapshot), withdrawing the tokens and finally sending the tokens back into the flash loan pool.
  // The user has way more tokens than the users, so they effectively cause their rewards to round to 0
  it("Execution", async function () {
    // Increase time until a new rewards round
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
