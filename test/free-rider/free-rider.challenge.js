...
describe("[Challenge] Free Rider", function () {
  ...
  it("Execution", async function () {
    ...
    // 1. Take out a uniswap flash-swap to cover the NFT purchase price
    // 2. Swap the WETH -> ETH
    // 3. Execute buyMany, providing the NFT_PRICE as the value and all the tokenId's for sale (the bug in the _buyOne method line:103 will mean that the caller will not be deducted the ETH amount for purchasing)
    // 4. Transfer nft's to FreeRiderRecovery
    // 5. Bug bounty will be transferred to the exploit contract
    // 6. Pay back uniswap
    // 7. Send ETH to caller
    ...
    await exploitContract
      .connect(player)
      // Pass enough to cover the uniswap flash-swap fee
      .runExploit({ value: parseEther("0.05") });
  });
